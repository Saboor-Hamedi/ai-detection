...
const ForensicRouter = {
    views: ['editor', 'archive', 'visual-pdf'],
    ...
    switchView(view, push = true) {
        ...
        const views = {
            ...
            visual: document.getElementById('view-visual-pdf')
        };
        ...
        const tabs = {
            ...
            visual: document.getElementById('tab-visual-pdf')
        };
        ...
        // Reset Nav Classes
        const navBase = "group w-full flex items-center p-2 text-slate-400 hover:text-slate-900 rounded-md text-[10px] font-bold uppercase tracking-widest text-left transition-colors";
        Object.values(navs).forEach(n => { if(n) n.className = navBase; });

        // Reset Tab Classes
        const tabBase = "text-[10px] font-black text-slate-400 uppercase tracking-widest border-b-2 border-transparent pb-1";
        ...
        if (view === 'editor') {
            if (tabs.editor) tabs.editor.className = "text-[10px] font-black text-slate-900 uppercase tracking-widest border-b-2 border-slate-900 pb-1";
            if (navs.editor) navs.editor.className = "group w-full flex items-center p-2 bg-slate-50 rounded-md text-slate-900 text-[10px] font-bold uppercase tracking-widest text-left";
        } else if (view === 'archive') {
            if (navs.archive) navs.archive.className = "group w-full flex items-center p-2 bg-slate-50 rounded-md text-slate-900 text-[10px] font-bold uppercase tracking-widest text-left";
            if (window.ForensicEngine) window.ForensicEngine.renderHistory();
        } else if (view === 'visual-pdf') {
            if (tabs.visual) tabs.visual.className = "text-[10px] font-black text-slate-900 uppercase tracking-widest border-b-2 border-slate-900 pb-1";
        }
